...
ExplodingSprite::ExplodingSprite(const Sprite& s) :
  ...
  freeList(),
  surfaces(),
  frames(),expSpeedX(Gamedata::getInstance().getXmlInt(s.getName()+"/speedX")),expSpeedY(Gamedata::getInstance().getXmlInt(s.getName()+"/speedY")){
...
void ExplodingSprite::makeChunks(unsigned int n) { 
  ...
  int speedx = expSpeedX;//static_cast<int>(velocityX()); // Wanna test for zero...
  int speedy = expSpeedX;//static_cast<int>(velocityY()); // Make sure it's an int.
  ...
  while  (source_y < getFrame()->getHeight() ) {
    ...
    while ( source_x < getFrame()->getWidth() ) {
      ...
      float sx = (rand() % speedx + 140) * (rand()%2?-1:1); // 'cause %0 is 
