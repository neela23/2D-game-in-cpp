...
Health::Health() :
  ...
  start(Vector2f(Gamedata::getInstance().getXmlInt("health/startX"), Gamedata::getInstance().getXmlInt("health/startY"))), 
  totalLength(200), 
  currentLength(200), 
  thick(14), 
  increments(50),
  ...
  GREEN( SDL_MapRGB(screen->format, 0x00, 0x64,0x00) ),
  color(GREEN) {
}
...
void Health::update(Uint32 ticks) {
  ...
  if ( currentLength > 0 && deltaTime > interval ) {
   ...
   if( currentLength-increments <= 0)
    color = RED;
}

bool Health::isHealthZero() {
  if(currentLength <= 0)
     return true;
  else
  return false;
