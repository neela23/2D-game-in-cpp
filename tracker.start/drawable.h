...
public:
  ...
  virtual bool isEnemyCollided(const Drawable*) const { 
    throw std::string("No collidedWith");  
  }
  ...
  virtual void shoot(){ throw name+std::string("No bullets");}
  virtual void reset(){ throw name+std::string("Nothing to reset");}
